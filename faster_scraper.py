...
def get_lastest_videos(keyword, start_time, end_time):
    ...
    while(page_num <= MAX_PAGE_NUM and not videos_too_old):
        ...
        # if the last video on this page is posted after the end_time,
        # just skip this page.
        # if len(bv_list) > 0 and bili.get_basic_video_stat(bv_list[-1])['data']['pubdate'] > end_time:
        #     page_num += 1
        #     print(page_num)
    ...
    with bv_file_lock:
        with open("FINAL_BV_LIST_append.txt", "a") as file:
            for bv in videos:
                file.write(bv + "\n")
